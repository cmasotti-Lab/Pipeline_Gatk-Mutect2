...
wd <- "D:/Github-Projects/Pipeline_Gatk-Mutect2/Result_Mutect.ROP.2024-06-14/"
# wd <- "/media/vande/HDD/PROJETOS-HSL_BP/resultados_Mutect2-2023/"
setwd(wd)
dir.create("output-Mutect2_SomaticFilters.2024-06-14")
output <- "output-Mutect2_SomaticFilters.2024-06-14/"

#==============================================================================#
# STEP 0 -  Datasets reorganization ####
#==============================================================================#
snps<- fread(file="all.ROP_annotated.PASS.tsv", header=F, sep="\t", na.strings=c(".","NA"), fill=TRUE, check.names = FALSE)
snps.header<- fread(file="all.ROP_annotated.txt", nrows = 1,header=T, sep="\t", na.strings=c(".","NA"), fill=TRUE, check.names = FALSE)
colnames(snps) <- c("FILE_NAME", colnames(snps.header[,c(2:36)]),"Otherinfo1","Otherinfo2","Otherinfo3","CHR","POS","ID","REF","ALT","QUAL","FILTER", "INFO", "FORMAT", "GENOTYPE")

# Creating INDEX (CHR,POS,REF,ALT) and select the main columns
#==============================================================================#

index <- paste(snps$CHR, snps$POS, snps$REF, snps$ALT, sep="_")
snps <- cbind(index, snps)
dim(snps)   #111217     50 
# fwrite(snps, paste0(output,"/tab0.tsv"), quote = F, sep="\t")
...
tab1$SAMPLE <- tab1$FILE_NAME
tab1<- separate(data= tab1, col=SAMPLE, into=c("SAMPLE"), sep = ".dedup.tags.bqsr.bam")
tab1<- separate(data= tab1, col=FILE_NAME, into=c("EXOME_ID"), sep = "-ExC85")
length(unique(tab1$EXOME_ID))
tab1 <- subset(tab1, (!EXOME_ID %in% samples.excl))
length(unique(tab1$EXOME_ID)) # 72
length(unique(tab1$index))   # 74166
...
#==============================================================================#
unique(tab1$FORMAT) #"GT:AD:AF:DP:F1R2:F2R1:FAD:SB"    "GT:AD:AF:DP:F1R2:F2R1:FAD:PGT:PID:PS:SB"
FORMAT<-strsplit(unique(tab1$FORMAT), ":")[[2]]
FORMAT
# Separate genotype information
tab2<- separate(data = tab1, col = GENOTYPE, into =FORMAT, sep = ":")
...
length(unique(all[,"index"]))             # 74166
length(unique(all$SAMPLE))              # 72
...
#==============================================================================#
all$freq_gnomAD_status[is.na(all$gnomad41_exome_AF)] <- "not-reported"
all$freq_gnomAD_status[all$gnomad41_exome_AF<0.005] <- "rare"
all$freq_gnomAD_status[all$gnomad41_exome_AF>=0.005] <- "not-rare"
...
#==============================================================================#
all <- separate(data = all, col = cosmic99, into = c("COSMIC_ID","OCCURENCE"), sep = "OCCURENCE=")
...
all <- dplyr::select(all, !starts_with("V_"))
...
length(unique(all.hla$index))    #88 mut em HLA
...
length(unique(all[shared2exclude,"index"]))  #9094 
...
length(unique(all[shared2exclude,"index"]))  # 1827 
...
gridExtra::grid.arrange(p, bottom="COV distribution, 74.166 Mutations from 72 samples")
...
aux1<-as.data.frame(table(all$MAF), stringsAsFactors = F)
aux1$Var1<-as.numeric(aux1$Var1)
p<-ggplot(data=aux1, aes(x=Var1, y=Freq))+
  geom_col(width = .005) + 
  geom_vline(xintercept= 0.2, linetype="dashed", color = "red")+ theme_classic()+
  theme( axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1),legend.position="top")  
gridExtra::grid.arrange(p, bottom="MAF distribution, 74.166 Mutations from 72 samples")



#==============================================================================#
#    QUEM SÃO AS MUTAÇÕES COM MAF < 0.2 ?       ####
#==============================================================================#
## Quantifica MUTAÇOES not-rare gnomAD
#==============================================================================#
not_rare2exclude <- which(all$freq_gnomAD_status=="not-rare")
length(unique(all[not_rare2exclude,"index"]))   # 3316
## Quantifica MUTAÇOES not-rare no Abraom
#==============================================================================#
not_rareAbraom2exclude <- which(all$freq_abraom_status=="not-rare")
length(unique(all[not_rareAbraom2exclude,"index"])) #3611 
## FILTRO: REMOVE mutações not-rare no gnomAD ou Abraom ####
#==============================================================================#
all.exclMAF <- all[c(not_rare2exclude,not_rareAbraom2exclude),]
length(unique(all.exclMAF$index))  # 3990
# Quantificando regiões das mutações
#==============================================================================#
aux<-(unique(all.exclMAF[,c("index", "Func.refGene")]))  
as.data.frame(table(aux$Func.refGene))
# Quantificando tipos de mutações
#==============================================================================#
aux<-(unique(all.exclMAF[,c("index", "ExonicFunc.refGene")]))  
as.data.frame(table(aux$ExonicFunc.refGene))
# Quantificando mutações compartilhadas >=2
#==============================================================================#
shared2exclude <- which((all.exclMAF$MUTATION_shared >= 2) )
length(unique(all.exclMAF[shared2exclude,"index"]))  #885
# Quantificando mutações compartilhadas >=2, com menos de 4 registros no COSMIC?
#==============================================================================#
dgenes<-fread("input_Somatic-Filter/driver-genes.txt", header = F, col.names = "gene")
shared2exclude <- which((all.exclMAF$MUTATION_shared >= 2)  & (all.exclMAF$COSMIC_OCCURENCE <= 3))
length(unique(all.exclMAF[shared2exclude,"index"]))  # 1827 

length(unique(all.exclMAF[,"index"]))             # 74166
length(unique(all.exclMAF$SAMPLE))              # 72

aux<-(unique(all.exclMAF[,c("index", "COV")]))  
summary(aux$COV)

aux1<-as.data.frame(table(all.exclMAF$COV), stringsAsFactors = F)
aux1$Var1<-as.numeric(aux1$Var1)
p<-ggplot(data=head(aux1, 500), aes(x=Var1, y=Freq))+
  geom_col() + 
  geom_vline(xintercept= 30, linetype="dashed", color = "red")+theme_classic()+
...
length(unique(all.x1$index))  # 70176
...
aux1<-as.data.frame(table(all.x1$MAF), stringsAsFactors = F)
aux1$Var1<-as.numeric(aux1$Var1)
p<-ggplot(data=aux1, aes(x=Var1, y=Freq))+
  geom_col(width = .005) + 
  geom_vline(xintercept= 0.2, linetype="dashed", color = "red")+ theme_classic()+
  theme( axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1),legend.position="top")  
gridExtra::grid.arrange(p, bottom="MAF distribution, 70.176 Mutations from 72 samples")
...
length(unique(all.x1[shared2exclude,"index"]))   # 7886
all.xg <- all.x1[-shared2exclude,]  
length(unique(all.xg[,"index"]))          #62290    
...
p<-ggplot(data=head(aux1, 500), aes(x=Var1, y=Freq))+
  ...
  geom_vline(xintercept= 30, linetype="dashed", color = "red")+theme_classic()+
  theme( axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1),legend.position="top")  
gridExtra::grid.arrange(p, bottom="COV distribution, 62.290 Mutations from 72 samples")

aux1<-as.data.frame(table(all.xg$MAF), stringsAsFactors = F)
aux1$Var1<-as.numeric(aux1$Var1)
p<-ggplot(data=aux1, aes(x=Var1, y=Freq))+
  geom_col(width = .005) + 
  geom_vline(xintercept= 0.2, linetype="dashed", color = "red")+ theme_classic()+
  theme( axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1),legend.position="top")  
gridExtra::grid.arrange(p, bottom="MAF distribution, 62.290 Mutations from 72 samples")
...
#==============================================================================#
#  QUANTIFICANDO MUTAÇÕES DO ALL MUTECT ANTES DOS FILTROS MAF E COV ####
#==============================================================================#
length(unique(all.mut[,"index"]))             # 62290
length(unique(all.mut$SAMPLE))              # 72

#  Quantify mutation in HLA genes
#==============================================================================#
genesHLA <- fread("input_Somatic-Filter/genesHLA.txt", col.names = "gene", header = F) 
all.hla<- subset(all.mut, (Gene.refGene %in% genesHLA$gene))
length(unique(all.hla$index))    # 52
...
p<-ggplot(data=aux1, aes(x=Var1, y=Freq))+
  ...
  geom_vline(xintercept= 0.2, linetype="dashed", color = "red")+ theme_classic()+
  theme( axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1),legend.position="top")  
gridExtra::grid.arrange(p, bottom="MAF distribution, from 72 samples")
# Quantificando Distribuição da COV 
#==============================================================================#
...
#somatic.72<-as.data.frame(fread(paste0(output,"/somatic_mutation.72samples.tsv"), header = T,  na.strings=c("NA"), fill=TRUE, check.names = FALSE))
tab.allSomaticMut<-as.data.frame(fread(paste0(output,"mutation.popFilt.shared_cosmic.tsv"), header = T,  na.strings=c("NA"), fill=TRUE, check.names = FALSE))
length(unique(tab.allSomaticMut$index))
aux1 <- select(subset(tab.allSomaticMut, (SAMPLE %in% Clinical$Sample)  ), SAMPLE, MAF)
...
length(unique(all.mut[hla2exclude,"index"]))  #  52
all.mut2 <- all.mut[-hla2exclude,]
length(unique(all.mut2$index))  #62238

## FILTRO: REMOVE MUTACOES  MINOR ALLELIC FREQ. < 0.20 ####
#==============================================================================#
maf2exclude <- which(all.mut2$MAF<0.20) 
length(unique(all.mut2[maf2exclude,"index"]))  #  34464
all.mut.maf <- all.mut2[-maf2exclude,]
length(unique(all.mut.maf$index))  #27817

## FILTRO: REMOVE MUTACOES COV < 30 ####
#==============================================================================#
cov2exclude <- which(all.mut.maf$COV<30) 
length(unique(all.mut.maf[cov2exclude,"index"]))  #  619
all.mut.cov <- all.mut.maf[-cov2exclude,]
length(unique(all.mut.cov$index))  # 27199
...
length(unique(all.Coding$index))  #27067
...
#==============================================================================#
#    QUEM SÃO AS 34.464 MUTAÇÕES COM MAF < 0.2 ?       ####
#==============================================================================#

length(unique(all.Coding[,"index"]))             # 27067
length(unique(all.Coding$SAMPLE))              # 72

maf2exclude <- which(all.mut2$MAF<0.20) 
length(unique(all.mut2[maf2exclude,"index"]))  #  34464
all.mut.maf.excl <- all.mut2[maf2exclude,]
length(unique(all.mut.maf.excl$index))  #34464

length(unique(all.mut.maf.excl[,"index"]))             # 34464
length(unique(all.mut.maf.excl$SAMPLE))              # 72

# Quantificando regiões das mutações
#==============================================================================#
aux<-(unique(all.mut.maf.excl[,c("index", "Func.refGene")]))  
as.data.frame(table(aux$Func.refGene))
# Quantificando tipos de mutações
#==============================================================================#
aux<-(unique(all.mut.maf.excl[,c("index", "ExonicFunc.refGene")]))  
as.data.frame(table(aux$ExonicFunc.refGene))
# Quantificando mutações compartilhadas >=2
#==============================================================================#
shared2exclude <- which((all.mut.maf.excl$MUTATION_shared >= 2) )
length(unique(all.mut.maf.excl[shared2exclude,"index"]))  #299
# Quantificando mutações compartilhadas >=2, com menos de 4 registros no COSMIC?
#==============================================================================#
dgenes<-fread("input_Somatic-Filter/driver-genes.txt", header = F, col.names = "gene")
shared2exclude <- which((all.mut.maf.excl$MUTATION_shared >= 2)  & (all.mut.maf.excl$COSMIC_OCCURENCE <= 3))
length(unique(all.mut.maf.excl[shared2exclude,"index"]))  # 201 

length(unique(all.mut.maf.excl[,"index"]))             # 34464
length(unique(all.mut.maf.excl$SAMPLE))              # 72

aux1<-as.data.frame(table(all.mut.maf.excl$MAF), stringsAsFactors = F)
aux1$Var1<-as.numeric(aux1$Var1)
p<-ggplot(data=aux1, aes(x=Var1, y=Freq))+
  geom_col(width = .005) + 
  geom_vline(xintercept= 0.2, linetype="dashed", color = "red")+ theme_classic()+
...
aux<-(unique(all.mut.maf.excl[,c("index", "COV")]))  
summary(aux$COV)

aux1<-as.data.frame(table(all.mut.maf.excl$COV), stringsAsFactors = F)
aux1$Var1<-as.numeric(aux1$Var1)
p<-ggplot(data=head(aux1, 500), aes(x=Var1, y=Freq))+
  geom_col() + 
  geom_vline(xintercept= 30, linetype="dashed", color = "red")+theme_classic()+
...
length(unique(somatic[,"index"]))     # 20062
...
somatic.72 <- somatic[!is.na(somatic$ID_Exoma),] 

### SALVAR A TABELA COM MUTACOES SOMATICAS DAS 72 amostras ####
#==============================================================================#
#fwrite(somatic.72, paste0(output,"/somatic_mutation.72samples.tsv"), quote = F, sep="\t")
...
aux<-(unique(somatic.72[,c("index", "ExonicFunc.refGene")]))  
...
gridExtra::grid.arrange(p, bottom="MATH-score correlation nCRT-R x nCRT-NR (70 samples)")
table(sampleTableCount.Clinical2$Response1)
...
p <- ggplot(sampleTableCount.Clinical2, aes(x=Response2, y=MATH_score, fill=Response2)) + 
  ...
  stat_compare_means(label.y = 80)+
  scale_fill_manual(values = c("#08415c","#cc2956", "#ef767a"), breaks = c("nCRT-R", "nCRT-NR Metastatic", "nCRT-NR Not-Metastatic")) +
  annotate("text", label = paste('N = 14', sep = ''), x = 3, y = 3, angle = 0, size = 3.75) +
  annotate("text", label = paste('N = 38', sep = ''), x = 2, y = 3, angle = 0, size = 3.75) +
  annotate("text", label = paste('N = 18', sep = ''), x = 1, y = 3, angle = 0, size = 3.75) +
  stat_compare_means(comparisons = my_comparisons)+
  theme_classic()+theme(legend.position="none")+
  geom_jitter(shape=16, position=position_jitter(0.2))
gridExtra::grid.arrange(p, bottom="MATH-score correlation Response2 (70 samples)")
...
p <- ggplot(sampleTableCount.Clinical  , aes(x=Response3, y=MATH_score, fill=Response3)) + 
  ...
  annotate("text", label = paste('N = 54', sep = ''), x = 2, y = 3, angle = 0, size = 3.75) +
  annotate("text", label = paste('N = 18', sep = ''), x = 1, y = 3, angle = 0, size = 3.75) +
  theme_classic()+theme(legend.position="none")+
  geom_jitter(shape=16, position=position_jitter(0.2))
gridExtra::grid.arrange(p, bottom="MATH-score correlation Response2 (72 samples)")
...
p <- ggplot(sampleTableCount.Clinical2  , aes(x=Response3, y=MATH_score, fill=Response3)) + 
  geom_boxplot(outlier.shape = NA)+ stat_compare_means()+ 
  scale_fill_manual(values = c("#cc2956", "#ef764a"), breaks = c("Metastatic", "Not-Metastatic")) +
  annotate("text", label = paste('N = 52', sep = ''), x = 2, y = 3, angle = 0, size = 3.75) +
  annotate("text", label = paste('N = 18', sep = ''), x = 1, y = 3, angle = 0, size = 3.75) +
  theme_classic()+theme(legend.position="none")+
  geom_jitter(shape=16, position=position_jitter(0.2))
gridExtra::grid.arrange(p, bottom="MATH-score correlation Response2 (70 samples)")
...
p <- ggplot(sampleTableCount.Clinical2  , aes(x=Response1, y=TMB, fill=Response1)) + 
  ...
  stat_compare_means()+ theme_classic()+theme(legend.position="none")+
  geom_jitter(shape=16, position=position_jitter(0.2))
gridExtra::grid.arrange(p, bottom="TMB correlation Response1 (70 samples)")
table(sampleTableCount.Clinical2$Response1)
...
p <- ggplot(sampleTableCount.Clinical2, aes(x=Response2, y=TMB, fill=Response2)) + 
  ...
  theme_classic()+theme(legend.position="none")+
  geom_jitter(shape=16, position=position_jitter(0.2))
gridExtra::grid.arrange(p, bottom="TMB correlation Response2 (70 samples)")
table(sampleTableCount.Clinical2$Response2)
...
p <- ggplot(sampleTableCount.Clinical, aes(x=Response3, y=TMB, fill=Response3)) + 
  ...
  stat_compare_means(label.y = 10)+
  scale_fill_manual(values = c("#cc2956", "#ef764a"), breaks = c("Metastatic", "Not-Metastatic")) +
  annotate("text", label = paste('N = 54', sep = ''), x = 2, y = 0, angle = 0, size = 3.75) +
  annotate("text", label = paste('N = 18', sep = ''), x = 1, y = 0, angle = 0, size = 3.75) +
  theme_classic()+theme(legend.position="none")+
  geom_jitter(shape=16, position=position_jitter(0.2))
...
p <- ggplot(sampleTableCount.Clinical2, aes(x=Response3, y=TMB, fill=Response3)) + 
  ...
  stat_compare_means(label.y = 10)+
  scale_fill_manual(values = c("#cc2956", "#ef764a"), breaks = c("Metastatic", "Not-Metastatic")) +
  annotate("text", label = paste('N = 52', sep = ''), x = 2, y = 0, angle = 0, size = 3.75) +
  annotate("text", label = paste('N = 18', sep = ''), x = 1, y = 0, angle = 0, size = 3.75) +
  theme_classic()+theme(legend.position="none")+
  geom_jitter(shape=16, position=position_jitter(0.2))
gridExtra::grid.arrange(p, bottom="TMB correlation Response3 (70 samples)")
table(sampleTableCount.Clinical2$Response3)
...
gridExtra::grid.arrange(sp, bottom="Spearman Test (52 samples) excluded Metastatic")
...
sp <- ggscatter(subset(sampleTableCount.Clinical2,Response2 != "nCRT-NR Metastatic"), x = "MATH_score", y = "TMB",
  ...
  geom_point(aes(color = Response2))+
  theme(legend.position="top")
gridExtra::grid.arrange(sp, bottom="Spearman Test (70 samples) excluded Metastatic")
# RESPONSE 3
sp <- ggscatter(sampleTableCount.Clinical2, x = "MATH_score", y = "TMB",
                add = "reg.line",  # Add regressin line
                cor.method = "spearman",
...
mut.genes=unique(dplyr::select(somatic.72, c("index","Gene.refGene")))
...
all.mut.flt.72<- subset(all.mut.flt, SAMPLE %in% unique(Clinical$Sample))
# 
...
mutations <- all.mut.flt.72[,c("SAMPLE", "Chr", "Start", "Ref", "Alt")]
dndsout = dndscv(mutations, refdb="D:/PROJETOS-HSL_BP/resultados_Mutect2-2023/input_Somatic-Filter/RefCDS_human_GRCh38.p12_dNdScv.0.1.0.rda", cv=NULL,
...
mutations3 <- all.mut.flt.72[,c("SAMPLE", "CHR", "POS", "REF", "ALT")]
...
fwrite(sel_cv[sel_cv$pglobal_cv<0.05, ], paste0(output,"/somatic.72.mut_signif_dndscv.tsv"), quote = F, sep="\t")
fwrite(signif_genes, paste0(output,"/somatic.72.mut_signif_genes.tsv"), quote = F, sep="\t")

#==============================================================================#
#                ONCOPLOT  72 samples ####
#==============================================================================#
signif_genes <- fread(file=paste0(output,"/somatic.72.mut_signif_genes.tsv"))

somatic.maf<-fread(paste0(output,"/mutation_codRegion_Freq005.tsv"), header = T,  na.strings=c("NA"), fill=TRUE, check.names = FALSE)
somatic.maf.72<- subset(somatic.maf, SAMPLE %in% unique(Clinical$Sample))
somatic.maf.72 <- merge(somatic.maf.72, sampleTableCount.Clinical[,c("SAMPLE","ID_Exoma")], by= "SAMPLE", all.x = T)
# somatic.maf<- subset(somatic.maf.72, select=c("CHR", "POS", "REF", "ALT", "Gene.refGene","ID_Exoma","ExonicFunc.refGene"))
somatic.maf<- subset(somatic.maf.72, select=c("Chr", "Start",  "End","Ref", "Alt", "Gene.refGene","ID_Exoma","ExonicFunc.refGene"))
...
# somatic.maf<- subset(somatic.72, select=c("Chr", "Start",  "End","Ref", "Alt", "Gene.refGene","ID_Exoma","ExonicFunc.refGene"))
# somatic.maf<- somatic.maf %>%
#   mutate(Gene.refGene = strsplit(as.character(Gene.refGene), ";")) %>%
#   unnest(Gene.refGene)
...
aux<-unique(subset(somatic.72, class == "dMMR",select = c("SAMPLE","class") ))
...
#==============================================================================#
mut.genes=unique(select(somatic.72, c("Gene.refGene","ID_Exoma", "class")))
aux<- as.data.frame(unique(subset(mut.genes, select= ID_Exoma)))

mut.genes.heatmap= as.data.frame(table(unique(select(somatic.72, c("Gene.refGene","ID_Exoma")))))
...
final_Sig$dMMR <-"pMMR"
mutados<- unique(subset(somatic.72, class == "dMMR", select = SAMPLE))
...
#==============================================================================#
# Fisher Test / Chisq Test - Drivers genes, dMMR ####
#==============================================================================#

sampleTableCount.Clinical$DriverGene <-"notDriver"
mutados<- unique(subset(somatic.72, class == "driver", select = ID_Exoma))
sampleTableCount.Clinical[sampleTableCount.Clinical$ID_Exoma %in% mutados$ID_Exoma,]$DriverGene <-"RectalDriver"

sampleTableCount.Clinical$dMMR <-"pMMR"
mutados<- unique(subset(somatic.72, class == "dMMR", select = ID_Exoma))
...
for (resp in respostas) {
  for (gene in dndsGenes$gene_name) {
    ...
    mutados<- unique(subset(somatic.72,  Gene.refGene == gene, select = ID_Exoma))
