#Pipeline atualizado em 02/01/2025
...
export WD="/home/scratch90/vlira_18nov2024/"

export MEM=100
export JOBS=5

export DATA=$(date "+%F")
export OUTPUT_DIR=${WD}"/Result_Mutect2.GATK4.6.ROP_CRAM.${DATA}"
...
export LOG_FILE="$OUTPUT_DIR.log"

mkdir $OUTPUT_DIR


ls $WD/preprocessing_TOY_result/*chr20.cram > $OUTPUT_DIR/samples.list.cram
export SAMPLE_LIST_BAM="$OUTPUT_DIR/samples.list.cram"

ls $WD/preprocessing_CRAM_result/*.cram > $OUTPUT_DIR/ROP.list.cram
export SAMPLE_LIST_BAM="$OUTPUT_DIR/ROP.list.cram"
...
stage_Mutect2 (){
  ...
  $GATK --java-options "-Xmx${MEM}G" Mutect2 \
        ...
        -bamout $OUTPUT_DIR/Mutect2/"${ID}.bamout.bam" \
