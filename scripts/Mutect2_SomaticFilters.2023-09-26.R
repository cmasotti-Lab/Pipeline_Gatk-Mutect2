...
setwd(wd)
dir.create("output-Mutect2_SomaticFilters.2023-09-26")
output <- "output-Mutect2_SomaticFilters.2023-09-26/"
...
#==============================================================================#
snps.flt<- select(snps, c(1:5, ends_with('.refGene'),"gnomAD_exome_ALL",starts_with("abraom"),"cosmic95", starts_with("ICGC"),"Interpro_domain", starts_with("Otherinf") ))
...
snps.flt <- select(snps.flt, index,all_of(exCol))
dim(snps.flt)   #371253 
...
## Quntifica MUTAÇOES not-rare gnomAD
#==============================================================================#
...
#==============================================================================#
maf2exclude <- which(all.mut$MAF<0.20) 
length(unique(all.mut[maf2exclude,"index"]))  #  27153
all.mut.maf <- all.mut[-maf2exclude,]
length(unique(all.mut.maf$index))  #29677
...
length(unique(all.mut.cov$index))  # 29137
...
length(unique(all.Coding$index))  #
...
length(unique(somatic[,"index"]))     # 21412
